...
* `LDAP_GROUP_ADDITIONAL_OBJECTCLASSES` (no default): A comma-separated list of additional objectClasses to use when creating an group.

* `LDAP_GROUP_ADDITIONAL_ATTRIBUTE` (no default): A comma-separated list of extra attributes to display when creating an group.
...
* `LDAP_ACCOUNT_ATTRIBUTES_PERSONAL` (no default): A comma-separated list of personal attributes to display when user editing.
...
* `$CUSTOM_LOGO` (default: *FALSE*): If this is defined with path to image file, then this image will be displayed in header. You need mount volume with this file.

* `$CUSTOM_STYLES` (default: *FALSE*):  If this is defined with path to css file, then this style will be used in header. Also helps with logo positioninig. You need mount volume with this file.
